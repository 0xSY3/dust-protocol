...
import { listTestingSubsetArtifacts, loadArtifacts } from '../../helpers/logic/artifacts';
...
task(
  ...
  .setAction(async function ({ weth9 }: { weth9: string }, hre) {
    ...
    // Set artifacts (use testing subset for testnet — fewer VKeys, less gas)
    console.log('\nSetting Artifacts (testing subset)');
    await loadArtifacts(railgun, listTestingSubsetArtifacts());
